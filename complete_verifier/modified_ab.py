...
from loading import load_model_and_vnnlib, custom_parse_run_mode, adhoc_tuning, Customized  # pylint: disable=unused-import
...
class ABCROWN:
    ...
    def main(self, interm_bounds=None):
        ...
        k = arguments.Config['k']
        
        q = deque([])
        
        image_path = arguments.Config['imagepath']
        image = Image.open(image_path)
        transform = transforms.ToTensor()
        test_img = transform(image)   
        shape = [-1] + list(test_img.shape)
        test_img = test_img.reshape(shape) 
        
        model_out = model_ori(test_img)
        label = torch.argmax(model_out,dim = 1)     
        ...
        # n*n tensor matrix of elements 0
        matrix = torch.zeros((n, n))

        print(f"Now starting verification for image {image_path} with label {label} from size {m}")
        ...
        while(m >= k):
            ...
            call_matrix = torch.zeros((n, n))
            
            for i in range(q_size):
                pos_x, pos_y = q.popleft()

                if (call_matrix[pos_x][pos_y] == 1):
                    continue
                
                call_matrix[pos_x][pos_y] = 1
                
                if (matrix[pos_x][pos_y] == 1):
                    #submatrix of size m/2 * m/2 make it's elements 1
                    new_matrix[pos_x:pos_x + (m//2) + 1, pos_y:pos_y + (m//2) + 1] = 1
                    continue

                verifier_call_count += 1

                print(f"Now calling verifier of size {m} for position {pos_x}, {pos_y}")

                bab_args = arguments.Config['bab']
                timeout_threshold = bab_args['timeout']
                
                (run_mode, save_path, file_root, vnnlib) = custom_parse_run_mode(test_img, m, pos_x, pos_y,label)
                ...
                if ("unsafe" in verified_status) or ("unknown" in verified_status) or ("timeout" in verified_status):
                    if (m == k):
                        if ("unsafe" in verified_status):
                            print(f'Model is Unsafe at {pos_x}, {pos_y} of size {k}')
                            return
                        else:
                            print(f'Model is Unknown at {pos_x}, {pos_y} of size {k} due to timeout')
                            print(f'Verifier call count: {verifier_call_count}')
                            return
                        ...
                        for i in range(9):
                            a = i // 3
                            b = i % 3
                            q.append((pos_x + a * (m//4), pos_y + b * (m//4)))
                    else:
                        for i in range((k + 1) ** 2):
                            a = i // (k + 1)
                            b = i % (k + 1)
                            q.append((pos_x + a, pos_y + b))
                        # q.append((pos_x, pos_y))

                
                else:
                    print(f'Model is Verified True at {pos_x}, {pos_y} of size {m}')
                    new_matrix[pos_x:pos_x + (m//2) + 1, pos_y:pos_y + (m//2) + 1] = 1

            m = m // 2
            matrix = new_matrix

        print("Verified for all positions, No unsafe pertubution")
        print(f'Verifier call count: {verifier_call_count}')
        # Summarize results.
