...
def custom_construct_vnnlib(dataset, example_idx_list, k, sp_x, sp_y):
    X = dataset['X']
    ...
    if arguments.Config['specification']['type'] == 'lp':
        ...
        if arguments.Config['specification']['norm'] == float('inf'):
            if dataset.get('data_max', None) is None:
                ...
                matrix = torch.zeros(X.shape[1:])
                matrix[:, sp_x:sp_x + k, sp_y:sp_y + k] = 1

                x_lower = (X[example_idx_list] - matrix * perturb_epsilon).clamp(
                    min=dataset['data_min']).flatten(1)
                x_upper = (X[example_idx_list] + matrix * perturb_epsilon).clamp(
    ...
    if arguments.Config['specification']['type'] == 'lp':
        ...
        if arguments.Config['specification']['norm'] == float('inf'):
            if dataset.get('data_max', None) is None:
                ...
                print("Shaape", X.shape)
                print("Eppppppp", X[0], perturb_epsilon, X[0] - perturb_epsilon)  
