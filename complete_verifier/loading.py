...
from specifications import construct_vnnlib, custom_construct_vnnlib
...
def custom_parse_run_mode(test_img, k, pos_x, pos_y,label):
    ...
    if (arguments.Config['general']['csv_name']
        ...
        if arguments.Config['specification']['vnnlib_path'] is None:
            ...
            # data_max = torch.tensor([[[[float('inf')]]]])
            # data_min = torch.tensor([[[[-float('inf')]]]])            
            ...
            ver_dataset = {
                ...
                'labels': label,
                'runnerup': None,
                'data_max': torch.tensor([[[[arguments.Config['model']['data_max']]]]]),
                'data_min': torch.tensor([[[[arguments.Config['model']['data_min']]]]]),
                'eps': torch.tensor([[[[arguments.Config['specification']['epsilon']]]]]),
                'norm': float(arguments.Config['specification']['norm']),
                'target_label': None
            }

            test_img = custom_construct_vnnlib(ver_dataset,[0], k, pos_x, pos_y)
    ...
    return run_mode, save_path, file_root, test_img
